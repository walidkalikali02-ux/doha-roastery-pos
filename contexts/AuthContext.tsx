...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const demoRole = (localStorage.getItem('demo_role') as UserRole) || UserRole.ADMIN;

    if (isDemo) {
      setState({
        user: {
          id: 'demo-user',
          email: `${demoRole.toLowerCase()}@roastery.demo`,
          name: demoRole === UserRole.ADMIN ? 'مدير النظام (تجريبي)' : 
                demoRole === UserRole.ROASTER ? 'مسؤول التحميص (تجريبي)' : 'كاشير المبيعات (تجريبي)',
          role: demoRole,
          permissions: getPermissionsForRole(demoRole),
        },
        isAuthenticated: true,
        isLoading: false,
        error: null,
        sessionExpiresAt: new Date(Date.now() + 86400000), // 24 hours
      });
      return;
    }
  ...
  const loginAsGuest = (role: UserRole) => {
    ...
    const demoUser = {
      id: 'demo-user',
      email: `${role.toLowerCase()}@roastery.demo`,
      name: role === UserRole.ADMIN ? 'مدير النظام (تجريبي)' : 
            role === UserRole.ROASTER ? 'مسؤول التحميص (تجريبي)' : 'كاشير المبيعات (تجريبي)',
      role: role,
