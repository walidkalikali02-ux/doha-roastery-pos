...
const RoastingView: React.FC<{ onDetailOpen?: (id: string | null) => void }> = ({ onDetailOpen }) => {
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-20">
      {showSuccess && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-8 py-3 rounded-full font-bold flex items-center gap-3 animate-in slide-in-from-top-4 shadow-xl z-[150] border border-green-200 dark:border-green-800">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-stone-800 dark:text-stone-100 flex items-center gap-3">
             <Database className="text-amber-600" /> {t.productionManagement}
          </h2>
          <p className="text-stone-500">{t.trackBatches}</p>
        </div>
        <button onClick={() => setShowNewBatchModal(true)} className="w-full sm:w-auto bg-amber-600 text-white px-6 py-3 rounded-xl shadow-lg hover:bg-amber-700 font-bold flex items-center justify-center gap-2 transition-all active:scale-95">
      ...
      {activeBatches.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-bold flex items-center gap-2"><Clock className="text-amber-500" /> {t.activeBatches}</h3>
            ...
            {activeBatches.map(batch => (
              <div key={batch.id} className="bg-white dark:bg-stone-900 rounded-[32px] p-6 shadow-sm border border-stone-100 dark:border-stone-800 transition-all hover:shadow-md">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="px-3 py-1 bg-orange-100 text-orange-600 rounded-full text-[10px] font-bold animate-pulse uppercase tracking-widest">{t.inProgress}</span>
                </div>
                ...
                {finishingBatchId === batch.id ? (
                  <div className="space-y-4">
                    <input type="number" step="0.01" autoFocus value={postWeightInput} onChange={e => setPostWeightInput(e.target.value)} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-xl px-4 py-3 font-mono font-bold outline-none focus:ring-2 focus:ring-amber-500" placeholder="Output kg" />
      ...
      <div className="bg-white dark:bg-stone-900 rounded-[40px] shadow-sm border border-stone-200 dark:border-stone-800 overflow-hidden">
        <div className="p-6 border-b border-stone-100 dark:border-stone-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-stone-50/50 dark:bg-stone-800/20">
          <div>
            <h3 className="font-bold flex items-center gap-2"><History size={18} className="text-amber-600" /> {lang === 'ar' ? 'سجل دفعات التحميص (المتاحة للتعبئة)' : 'Roasting Batches Log (Available for Packing)'}</h3>
          </div>
          <div className="flex flex-wrap gap-3">
             <button onClick={() => setShowOnlyReady(!showOnlyReady)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${showOnlyReady ? 'bg-amber-600 text-white shadow-lg' : 'bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 text-stone-600 dark:text-stone-400'}`}>
                <Package2 size={14} /> {lang === 'ar' ? 'جاهز للتغليف فقط' : 'Ready Only'}
             </button>
             <input type="text" placeholder={t.searchProduct} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-xl px-4 py-2 text-xs outline-none focus:ring-1 focus:ring-amber-500" />
          </div>
        </div>
          ...
          <table className={`w-full ${t.dir === 'rtl' ? 'text-right' : 'text-left'} text-sm`}>
            <thead className="bg-stone-50 dark:bg-stone-950 text-stone-500 uppercase tracking-widest text-[10px] font-bold border-b border-stone-100 dark:border-stone-800">
              <tr>
                ...
                <th className="px-8 py-5 text-amber-600">{t.weightRemaining}</th>
              ...
              {filteredHistory.map(batch => {
                ...
                return (
                  <tr key={batch.id} className="hover:bg-stone-50 dark:hover:bg-stone-800/40 cursor-pointer group transition-colors" onClick={() => setShowDetailsModal(batch)}>
                    <td className="px-8 py-5 font-mono font-bold text-stone-400 group-hover:text-amber-600">{batch.id}</td>
                    ...
                    <td className="px-8 py-5 font-black font-mono">
                       <span className={remaining < 1 ? 'text-amber-500' : 'text-amber-600'}>
                         {remaining.toFixed(2)}kg
                       </span>
                    </td>
                    <td className="px-8 py-5">
                       <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${isReady ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-600'}`}>
                          {isReady ? t.ready : t.completed}
                       </span>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-2">
                         {isReady && <button onClick={(e) => { e.stopPropagation(); handleOpenProduction(batch); }} className="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-xs font-bold shadow-md hover:bg-blue-700 active:scale-95 transition-all flex items-center gap-2"><Package2 size={14} /> {lang === 'ar' ? 'تغليف' : 'Package'}</button>}
                         <div className="p-1.5 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-400 group-hover:text-amber-600 transition-colors"><ArrowRight size={14} className={t.dir === 'rtl' ? 'rotate-180' : ''} /></div>
                      </div>
                    </td>
                  </tr>
                );
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
           <div className="bg-white dark:bg-stone-900 rounded-[40px] max-w-4xl w-full p-8 shadow-2xl animate-in zoom-in-95 overflow-y-auto max-h-[90vh] custom-scrollbar" dir={t.dir}>
              <div className="flex justify-between items-center mb-6">
                 <div>
                   <h3 className="text-2xl font-bold flex items-center gap-3"><Package2 className="text-blue-600" /> {lang === 'ar' ? 'توزيع وزن الدفعة' : 'Batch Packaging Allocation'}</h3>
                 </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div className="bg-stone-50 dark:bg-stone-800 p-5 rounded-3xl border border-stone-200 dark:border-stone-700">
                   <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-xs font-bold text-amber-600">{(getBatchWeightStats(showProductionModal).remaining - totalBatchPackagingWeightNeeded).toFixed(2)} kg متبقي</span>
                   </div>
                ...
                <div className={`p-5 rounded-3xl border transition-all ${totalBatchPackagingWeightNeeded > getBatchWeightStats(showProductionModal).remaining ? 'bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800' : 'bg-blue-50 border-blue-200 dark:bg-blue-900/10 dark:border-blue-800'}`}>
                   <span className="text-[10px] uppercase font-black text-stone-400 block mb-1">{t.weightNeeded}</span>
                   <span className={`text-2xl font-black font-mono ${totalBatchPackagingWeightNeeded > getBatchWeightStats(showProductionModal).remaining ? 'text-red-600' : 'text-blue-600'}`}>
              ...
              <form onSubmit={handleCreateProduction} className={`space-y-8 ${!hasProducts ? 'opacity-30 pointer-events-none grayscale' : ''}`}>
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div className="space-y-1">
                       ...
                       <input type="date" required value={productionDates.productionDate} onChange={e => setProductionDates({...productionDates, productionDate: e.target.value})} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-4 py-4 font-bold outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div className="space-y-1">
                       ...
                       <input type="date" required value={productionDates.packagingDate} onChange={e => setProductionDates({...productionDates, packagingDate: e.target.value})} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-4 py-4 font-bold outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div className="space-y-1">
                       ...
                       <select required value={productionDates.targetLocationId} onChange={e => setProductionDates({...productionDates, targetLocationId: e.target.value})} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-4 py-4 font-bold outline-none focus:ring-2 focus:ring-blue-500">
                 ...
                 <div className="space-y-4">
                    <div className="flex justify-between items-center">
                       ...
                       <button type="button" onClick={addPackagingLine} className="text-blue-600 hover:text-blue-700 font-bold text-xs flex items-center gap-1"><Plus size={14} /> {lang === 'ar' ? 'إضافة سطر منتج' : 'Add Product Line'}</button>
                    </div>
                          ...
                          return (
                             <div key={line.tempId} className={`flex flex-col md:flex-row gap-4 p-4 rounded-2xl border transition-all animate-in slide-in-from-top-2 duration-300 bg-stone-50 dark:bg-stone-800/50 border-stone-200 dark:border-stone-700`}>
                                <div className="flex-1 space-y-1">
                                   <select required value={line.productId} onChange={e => updatePackagingLine(line.tempId, 'productId', e.target.value)} className={`w-full bg-white dark:bg-stone-800 border-none rounded-xl px-3 py-3 font-bold outline-none focus:ring-2 focus:ring-blue-500`}>
                                ...
                                <div className="w-full md:w-28 space-y-1">
                                   <input type="number" required value={line.quantity} onChange={e => updatePackagingLine(line.tempId, 'quantity', e.target.value)} className="w-full bg-white dark:bg-stone-800 border-none rounded-xl px-3 py-3 font-mono font-bold outline-none focus:ring-2 focus:ring-amber-500" min="1" />
                                </div>
                                <div className="w-full md:w-28 space-y-1">
                                ...
                                <button type="button" onClick={() => removePackagingLine(line.tempId)} disabled={packagingLines.length === 1} className="p-3 text-stone-300 hover:text-red-500 disabled:opacity-30 transition-colors"><Trash2 size={20} /></button>
                             </div>
                          );
                       })}
                    </div>
                 ...
                 <div className="flex gap-4 pt-6">
                    ...
                    <button type="submit" disabled={isSaving || totalBatchPackagingWeightNeeded <= 0 || totalBatchPackagingWeightNeeded > getBatchWeightStats(showProductionModal).remaining} className="flex-[2] py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-xl flex items-center justify-center gap-2 hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-50">
           ...
           <div className="bg-white dark:bg-stone-900 rounded-[40px] max-w-4xl w-full shadow-2xl overflow-hidden relative my-8" dir={t.dir}>
              <div className="p-8 border-b border-stone-100 dark:border-stone-800 flex justify-between items-center bg-stone-50/50 dark:bg-stone-800/40">
                 <div className="flex items-center gap-5">
                    <div className="p-4 bg-amber-600 text-white rounded-3xl shadow-lg"><Coffee size={32} /></div>
              ...
              <div className="p-8 space-y-10">
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-amber-50 dark:bg-amber-900/10 p-6 rounded-[32px] border border-amber-100 dark:border-amber-900/30"><span className="text-[10px] font-black uppercase text-amber-600 block mb-2">{t.wasteRatio}</span><span className="text-2xl font-black font-mono text-amber-600">{showDetailsModal.wastePercentage}%</span></div>
                 </div>
                 <div className="space-y-4">
                    <h4 className="text-sm font-bold flex items-center gap-2 text-blue-600"><Layers size={18} /> {t.packagingUnits}</h4>
                       ...
                       <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {showDetailsModal.packagingUnits.map((unit) => (
                             <div key={unit.id} className="bg-white dark:bg-stone-950 border border-stone-200 dark:border-stone-800 rounded-2xl p-5 group flex items-center gap-4">
                                <div className="p-3 bg-stone-50 dark:bg-stone-900 rounded-xl"><QrCode className="text-stone-400 group-hover:text-blue-600 transition-colors" size={24} /></div>
                                ...
                                <button onClick={() => setShowPrintLabelModal(unit)} className="p-3 bg-stone-100 dark:bg-stone-800 text-stone-400 hover:text-amber-600 rounded-xl transition-all"><Printer size={20} /></button>
                             </div>
                          ))}
                       </div>
        ...
        <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
           <div className="bg-white dark:bg-stone-900 rounded-[40px] max-w-md w-full p-8 shadow-2xl relative" dir={t.dir}>
              <div className="flex justify-between items-center mb-8 border-b border-stone-100 dark:border-stone-800 pb-4">
                 <h3 className="text-xl font-bold flex items-center gap-2"><Printer className="text-amber-600" /> {t.printPreview}</h3>
              ...
              <div id="label-to-print" className="bg-white border-2 border-stone-300 rounded-lg p-6 flex flex-col items-center text-stone-900 shadow-inner mb-8">
                 <div className="w-full flex justify-between items-start mb-4">
                    <div className="bg-amber-600 text-white p-1 rounded font-black text-[8px] tracking-tighter uppercase">Doha Roastery</div>
              ...
              <div className="flex gap-4">
                 ...
                 <button onClick={() => { window.print(); setShowPrintLabelModal(null); }} className="flex-[2] py-4 bg-amber-600 text-white rounded-2xl font-bold shadow-xl flex items-center justify-center gap-2 hover:bg-amber-700 active:scale-95 transition-all"><Printer size={20} /> {t.printNow}</button>
              </div>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
          <div className="bg-white dark:bg-stone-900 rounded-[40px] max-w-xl w-full p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-2xl font-bold flex items-center gap-3"><Flame className="text-amber-600" /> {t.startNewBatch}</h3>
            ...
            <form onSubmit={handleStartBatch} className="space-y-6" dir={t.dir}>
              <div className="space-y-1">
                ...
                <select required value={newBatchData.beanId} onChange={e => setNewBatchData({...newBatchData, beanId: e.target.value})} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-4 py-4 font-bold outline-none focus:ring-2 focus:ring-amber-500">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button type="submit" disabled={isSaving} className="flex-[2] py-4 bg-amber-600 text-white rounded-2xl font-bold shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all">
