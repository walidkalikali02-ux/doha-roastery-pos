...
NOTIFY pgrst, 'reload schema';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      {showSuccess && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 bg-black dark:bg-white text-white dark:text-black px-8 py-3 rounded-full font-bold flex items-center gap-3 shadow-xl z-[150] border border-stone-200 dark:border-stone-800">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-black dark:bg-white text-white dark:text-black rounded-[20px] shadow-lg"><Settings size={28} /></div>
          <div>
        ...
        <button onClick={() => { resetProductForm(); setShowProductModal(true); }} className="w-full md:w-auto bg-black dark:bg-white text-white dark:text-black px-8 py-3.5 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all hover:bg-stone-800 dark:hover:bg-stone-200">
      ...
      <div className="flex flex-wrap gap-4 bg-stone-100 dark:bg-stone-800/50 p-2 rounded-2xl w-fit mb-10">
        {['catalog', 'templates', 'settings', 'database', 'profile'].map(tab => (
          <button 
            ...
            className={`px-6 py-2.5 rounded-xl text-xs font-bold transition-all ${activeSubTab === tab ? 'bg-white dark:bg-stone-900 text-black dark:text-white shadow-sm border border-stone-200 dark:border-stone-800' : 'text-stone-500 hover:text-stone-700'}`}
          >
              ...
              <div className="flex-1 p-6 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <span className="text-[10px] text-stone-500 font-black uppercase">{product.category}</span>
                  </div>
                ...
                <div className="mt-auto flex justify-end">
                  ...
                  }} className="p-2.5 text-black dark:text-white hover:bg-stone-100 dark:hover:bg-stone-800 rounded-xl transition-all"><Edit3 size={18} /></button>
                </div>
              </div>
            </div>
          ))}
      ...
      {activeSubTab === 'database' && (
        <div className="bg-stone-900 rounded-[40px] p-10 text-white">
          <h3 className="text-2xl font-black mb-4 flex items-center gap-3 text-white"><Terminal size={24}/> SQL Schema Upgrade</h3>
          ...
          <button onClick={() => { navigator.clipboard.writeText(sqlFixScript); setCopyingSql(true); setTimeout(()=>setCopyingSql(false), 2000); }} className="bg-white/10 hover:bg-white/20 px-6 py-3 rounded-xl text-sm font-bold flex items-center gap-2">{copyingSql ? <CheckCircle size={18} className="text-white"/> : <Copy size={18}/>} Copy SQL Script</button>
        </div>
      )}
        ...
        <div className="max-w-4xl animate-in slide-in-from-bottom-4">
          <form onSubmit={handleSaveSettings} className="bg-white dark:bg-stone-900 rounded-[40px] p-10 border border-stone-200 dark:border-stone-800 shadow-sm space-y-10">
            <div className="flex items-center gap-4 border-b border-stone-100 dark:border-stone-800 pb-8">
              <div className="p-4 bg-stone-100 dark:bg-stone-800 text-black dark:text-white rounded-2xl"><Settings size={32} /></div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="flex bg-stone-100 dark:bg-stone-800 p-1 rounded-2xl w-fit">
                    <button type="button" onClick={() => setSettings({...settings, printer_width: '58mm'})} className={`px-8 py-3 rounded-xl font-bold text-sm transition-all ${settings.printer_width === '58mm' ? 'bg-white dark:bg-stone-900 text-black dark:text-white shadow-sm' : 'text-stone-500'}`}>{t.width58mm}</button>
                    <button type="button" onClick={() => setSettings({...settings, printer_width: '80mm'})} className={`px-8 py-3 rounded-xl font-bold text-sm transition-all ${settings.printer_width === '80mm' ? 'bg-white dark:bg-stone-900 text-black dark:text-white shadow-sm' : 'text-stone-500'}`}>{t.width80mm}</button>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest">{t.tax} (%)</label>
                  <input type="number" step="0.01" value={settings.vat_rate * 100} onChange={e => setSettings({...settings, vat_rate: parseFloat(e.target.value) / 100})} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-6 py-4 font-bold outline-none focus:ring-2 focus:ring-black dark:focus:ring-white" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-6 py-4 font-bold outline-none focus:ring-2 focus:ring-black dark:focus:ring-white" 
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input type="text" value={settings.store_name} onChange={e => setSettings({...settings, store_name: e.target.value})} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-6 py-4 font-bold outline-none focus:ring-2 focus:ring-black dark:focus:ring-white" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest">{lang === 'ar' ? 'عنوان المتجر' : 'Store Address'}</label>
                  <input type="text" value={settings.store_address} onChange={e => setSettings({...settings, store_address: e.target.value})} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-6 py-4 font-bold outline-none focus:ring-2 focus:ring-black dark:focus:ring-white" />
                </div>
              </div>
            </div>

            <div className="pt-6 flex justify-end">
              <button type="submit" disabled={isSaving} className="bg-black dark:bg-white text-white dark:text-black px-10 py-4 rounded-2xl font-black shadow-xl active:scale-95 transition-all flex items-center gap-3">
        ...
        <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md overflow-y-auto">
          <div className="bg-white dark:bg-stone-900 rounded-[40px] max-w-5xl w-full p-8 md:p-12 shadow-2xl animate-in zoom-in-95 my-8" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            <div className="flex justify-between items-center mb-10 border-b border-stone-100 pb-8">
              <div className="flex items-center gap-4">
                <div className="p-4 bg-stone-100 dark:bg-stone-800 text-black dark:text-white rounded-2xl"><Tag size={32} /></div>
            ...
            <form onSubmit={handleSaveProduct} className="space-y-10">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="space-y-8">
                  <div className="flex bg-stone-100 dark:bg-stone-800 p-1 rounded-2xl w-fit">
                    <button type="button" onClick={() => setProductForm({...productForm, type: 'PACKAGED_COFFEE'})} className={`px-6 py-3 rounded-xl font-bold text-xs transition-all ${productForm.type === 'PACKAGED_COFFEE' ? 'bg-white dark:bg-stone-900 text-black dark:text-white shadow-sm' : 'text-stone-500'}`}>{lang === 'ar' ? 'بن معبأ' : 'Packaged'}</button>
                    <button type="button" onClick={() => setProductForm({...productForm, type: 'BEVERAGE'})} className={`px-6 py-3 rounded-xl font-bold text-xs transition-all ${productForm.type === 'BEVERAGE' ? 'bg-white dark:bg-stone-900 text-black dark:text-white shadow-sm' : 'text-stone-500'}`}>{lang === 'ar' ? 'مشروب' : 'Beverage'}</button>
                  </div>

                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest">{t.productName}</label>
                    <input type="text" required value={productForm.name} onChange={e => setProductForm({...productForm, name: e.target.value})} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-6 py-4 font-bold outline-none focus:ring-2 focus:ring-black dark:focus:ring-white" />
                  </div>

                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest">{t.sellingPrice}</label>
                      <input type="number" step="0.01" required value={productForm.basePrice} onChange={e => setProductForm({...productForm, basePrice: e.target.value})} className="w-full bg-stone-50 dark:bg-stone-800 border-none rounded-2xl px-6 py-4 font-mono font-bold text-black dark:text-white" />
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-6">
                      <div className="space-y-4">
                        <div className="flex justify-between items-center border-b border-stone-100 pb-2">
                          ...
                          <button type="button" onClick={addIngredient} className="text-black dark:text-white font-bold text-xs flex items-center gap-1"><PlusCircle size={14}/> {lang === 'ar' ? 'إضافة مكون' : 'Add Item'}</button>
                        </div>
                        <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar">
                          {recipeIngredients.map((ing, idx) => (
                            <div key={idx} className="flex gap-2 items-center">
                              ...
                              <button type="button" onClick={() => setRecipeIngredients(recipeIngredients.filter((_, i) => i !== idx))} className="text-stone-300 hover:text-stone-500"><MinusCircle size={18}/></button>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center border-b border-stone-100 pb-2">
                          ...
                          <button type="button" onClick={addAddOn} className="text-black dark:text-white font-bold text-xs flex items-center gap-1"><Plus size={14}/> {lang === 'ar' ? 'إضافة خيار' : 'Add Option'}</button>
                        </div>
                        <div className="space-y-3">
                          {productAddOns.map((ao, idx) => (
                            <div key={ao.id} className="flex gap-2 items-center animate-in slide-in-from-top-1">
                              ...
                              <input type="number" placeholder="Price" value={ao.price} onChange={e => updateAddOn(idx, 'price', parseFloat(e.target.value))} className="w-24 bg-stone-50 dark:bg-stone-800 border-none rounded-xl px-3 py-2 text-xs font-mono font-bold text-black dark:text-white" />
                              <button type="button" onClick={() => setProductAddOns(productAddOns.filter((_, i) => i !== idx))} className="text-stone-300 hover:text-stone-500"><Trash2 size={16}/></button>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                <div className="bg-stone-900 rounded-[40px] p-10 text-white space-y-8 shadow-inner border border-white/5 relative overflow-hidden">
                   <h4 className="text-xl font-black flex items-center gap-3 border-b border-white/10 pb-6"><Calculator size={24} className="text-white" /> {lang === 'ar' ? 'تحليل التكلفة الذكي' : 'Smart Cost Analysis'}</h4>
                   ...
                   {productForm.type === 'BEVERAGE' ? (
                     <div className="space-y-6">
                        <div className="bg-white/5 p-6 rounded-3xl border border-white/10">
                           ...
                           <div className="flex justify-between items-end">
                              <span className="text-4xl font-black font-mono text-white">{calculatedBeverageCost.toFixed(2)}</span>
                     ...
                     <div className="space-y-6">
                        <div className="space-y-2">
                           ...
                           <input type="number" step="0.01" value={productForm.estimatedGreenBeanCost} onChange={e => setProductForm({...productForm, estimatedGreenBeanCost: e.target.value})} className="w-full bg-white/5 border-none rounded-2xl px-6 py-4 font-mono font-bold text-white outline-none" />
                        </div>
                        <div className="grid grid-cols-2 gap-6">
                           <div className="space-y-2">
              ...
              <div className="pt-10 flex gap-6 border-t border-stone-100 dark:border-stone-800">
                ...
                <button type="submit" disabled={isSaving} className="flex-[3] bg-black dark:bg-white text-white dark:text-black py-5 rounded-[24px] font-black shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-4 text-xl">
